...
from keyboards_my.note_kb import main_note_kb
...
async def edit_note_text_process(call: CallbackQuery, state: FSMContext):
    ...
    await call.message.answer(f'Отправь новое текстовое содержимое для заметки с ID {note_id}')
...
@upd_note_router.callback_query(F.data.startswith('dell_note'))
